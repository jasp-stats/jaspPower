...
.populatePowerESTabTest2Var <- function(jaspResults, options, r, lst) {
  ...
  str <- paste0(
    ...
    "<p>",
    gettextf(
      "To evaluate the design specified in the table, we can consider how sensitive it is to true effects of increasing sizes; that is, are we likely to correctly conclude that %1$s when the variance ratio is extreme enough to care about?",
      hypo_text
    ),
    "</p>"
  )
...
.populatePowerTabTest2Var <- function(jaspResults, options, r, lst) {
  ...
  if (calc == "sampleSize") {
    power_rounded <- round(.pwr2Var2NTest(n = n1, n.ratio = n_ratio, rho = d, sig.level = alpha, alternative = alt)$power, 3)
    if (power_rounded == 1) {
      power_rounded <- ">0.999"
    }
...
.populateContourTextTest2Var <- function(jaspResults, options, r, lst) {
  ...
  str <- paste(
    "<p>",
    gettext(
      "The power contour plot shows how the sensitivity of the test changes with the hypothetical variance ratio and the sample sizes in the design. As we increase the sample sizes, less extreme variance ratios become reliably detectable."
    ),
    "</p>",
    "<p>",
    gettext(
      "Conversely, if one is satisfied to reliably detect only more extreme variance ratios, smaller sample sizes are needed. The point shows the power of the specified design and variance ratio."
    ),
    "</p>"
  )
